...
- **New Charge → Customer** (2026-06-22): "Select / add customer" picks from (or adds to) the customer database; **Cash** can stay anonymous, **Card & Check require a customer** (block + prompt if none). The **closeout/transaction-history link** at the bottom is hidden for technician/front-desk (manager+).
- **Vendor tools** (2026-06-22): **Technician** sees the *Keycodes* tile only (no Vendors/NASTF tiles). **Front-desk** sees **no** "Vendor tools" section at all (no keycodes, vendors, or NASTF). **Manager/owner** see everything.
