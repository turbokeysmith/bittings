...
> generator was updated to match all of the above so a regenerate won't revert it. ⬜ **File cleanup pending owner
> review** — see `CLEANUP_INVENTORY.md` at the Desktop root (redundant old copies/zips; the active Bittings staff
> app is explicitly NOT touched). ⏸️ Payments still in **TEST mode** (separate track from the website launch).
