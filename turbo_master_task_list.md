...
### 🔮 PHASE 2 (captured, NOT started) — Commission engine + **POS / Cash-Register screen** (the centerpiece)
- Replace the "type-in-an-amount" charge box with a **real register**: a running ticket of line items (parts + services), each qty × price, **auto-calculating subtotal/tax/total**; parts pulled from inventory at **sell price** and **decrementing stock when sold**; services from a **manager-editable price list**; charges through the existing Stripe/cash flow.
- Respects today's rules (sell price only for techs/front-desk; pricing/discount = manager+; all staff take payment). **The ticket feeds the commission engine** — that's why they pair. **The G6 manager sign-off / reconciliation screen belongs here.**
